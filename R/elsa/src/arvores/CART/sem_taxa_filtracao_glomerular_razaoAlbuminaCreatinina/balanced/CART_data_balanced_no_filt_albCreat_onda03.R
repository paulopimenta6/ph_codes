...
dadosOnda3 <- data.frame(
  hipertensao = data$hip_onda3,
  potassio = data$pot_onda3,
  sodio = data$sod_onda3,
  PAS = data$PAS_onda3,
  PAD = data$PAD_onda3
)
### Verificar estrutura dos dados
str(dadosOnda3)  # Garantir que todas as variáveis preditoras são numéricas
################################################################################
...
response <- as.factor(ifelse(dadosOnda3$hipertensao == 'N', 0, 1))
predictors <- dadosOnda3[, -which(names(dadosOnda3) == "hipertensao")]
################################################################################
...
mod <- rpart(hipertensao ~ ., data = train)
################################################################################
# Plot da árvore
