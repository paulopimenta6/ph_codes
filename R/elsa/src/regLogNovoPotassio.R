...
################################################################################
## #Criacao de modelo
mod <- glm(hip_onda1 ~ pot_onda1, family = binomial(link = 'logit'), data = dadosOnda1)
plot(mod, which = 5)
summary(stdres(mod))
...
intlog <- dadosOnda1$pot_onda1 * log(dadosOnda1$pot_onda1)
dadosOnda1$intlog <- intlog
modint <- glm(hip_onda1 ~ pot_onda1 + intlog, family = binomial(link = 'logit'), data = dadosOnda1)
...
ggplot(dadosOnda1, aes(logito, pot_onda1)) +
  geom_point(size = 0.5, alpha = 0.5) +
  geom_smooth(method = "loess") +
  labs(x = "Logito", y = "Potassio na onda 1") +
  theme_classic()
