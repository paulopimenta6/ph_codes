...
library(pacman)
pacman::p_load(dplyr, psych, car, MASS, DescTools, QuantPsyc, ggplot2, survey, smotefamily)
source("./src/dadosRegLogistica.R")

dadosOnda2 <- data.frame(hip = dataGLM$hip_onda2,
                         pot = dataGLM$pot_onda2,
                         sod = dataGLM$sod_onda2
)
...
resultado_smote <- smotefamily::SMOTE(X = dadosOnda2[, -which(names(dadosOnda2) == "hip")],  # Dados sem a variável de resposta
                         target = as.factor(dadosOnda2$hip),                    # Variável de resposta como fator
                         K = 4,                                                 # Número de vizinhos
                         dup_size = 1.3)                                        # Multiplicação para minoritárias
...
dados_balanceados_smote <- rbind(dadosOnda2, dados_balanceados_smote)
